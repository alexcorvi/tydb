(function (global, factory) {
	typeof exports === "object" && typeof module !== "undefined"
		...
		: ((global = global || self), factory((global.tydb = {})));
    ...
    ***************************************************************************** */
	...
	function compareThings(a, b, _compareStrings) {
		...
		if (a instanceof Date) {
			return b instanceof Date
				? compareNSB(a.getTime(), b.getTime())
				: -1;
		}
	...
	comparisonFunctions.$all = function (a, b) {
		...
		if (!Array.isArray(b)) {
			throw new Error(
				"$all must be supplied with argument of type array"
			);
		}
			...
			if (dollarFirstChars.length > 0) {
				for (let i = 0; i < keys.length; i += 1) {
					if (!comparisonFunctions[keys[i]]) {
						throw new Error(
							"Unknown comparison function " + keys[i]
						);
					}
					if (
						!comparisonFunctions[keys[i]](
							objValue,
							queryValue[keys[i]]
						)
					) {
	...
	class BST {
		...
		checkInternalPointers() {
			if (this.left) {
				if (this.left.parent !== this) {
					throw new Error(
						`Parent pointer broken for key ${this.key}`
					);
				}
		...
		insert(key, value) {
			...
			if (!this.hasOwnProperty("key")) {
				this.key = key;
				if (typeof value !== "undefined") this.data.push(value);
				return;
			}
			// Same key as root
		...
		_searchNearestLte(key, nearestSoFar) {
			...
			if (this.compareKeys(key, this.key) < 0) {
				if (this.left) {
					const leftCandidate = this.left._searchNearestLte(
						key,
						nearest
					);
					if (
			...
			if (nearest == undefined || this.compareKeys(key, this.key) >= 0) {
				if (this.right) {
					...
					if (
						...
						(nearest == undefined ||
							Math.abs(
								this.compareKeys(rightCandidate.key, key)
							) < Math.abs(this.compareKeys(key, nearest.key))) &&
		...
		_searchNearestGte(key, nearestSoFar) {
			...
			if (this.compareKeys(key, this.key) < 0) {
				if (this.left) {
					const leftCandidate = this.left._searchNearestGte(
						key,
						nearest
					);
					if (
		...
		_searchNearest(key, nearestSoFar) {
			...
			if (this.compareKeys(key, this.key) < 0) {
				if (this.left) {
					const leftCandidate = this.left._searchNearest(
						key,
						nearest
					);
					if (
				...
				if (this.right) {
					const rightCandidate = this.right._searchNearest(
						key,
						nearest
					);
					if (
	...
	class Persistence {
		constructor(options) {
			...
			if (
				this.beforeDeserialization(
					this.afterSerialization(randomString$1)
				) !== randomString$1
			) {
				throw new Error(
						...
						yield emitter.emit(
							...
							this.afterSerialization(
								serialize({
									$$indexCreated: {
										fieldName,
										unique: this.db.indexes[fieldName]
											.unique,
										sparse: this.db.indexes[fieldName]
											.sparse,
									},
								})
							)
		...
		persistAllData() {
			return __awaiter(this, void 0, void 0, function* () {
				...
				for (let i = 0; i < allData.length; i++) {
					...
					yield emitter.emit(
						"writeLine",
						this.afterSerialization(serialize(doc))
					);
				}
				yield emitter.emit("end", "");
			});
		...
		loadDatabase() {
			return __awaiter(this, void 0, void 0, function* () {
				...
				let processed = 0;
				let err;
				indexesEmitter.on("readLine", (line) =>
					__awaiter(this, void 0, void 0, function* () {
						...
						if (treatedLine.type === "index") {
							if (treatedLine.status === "add") {
								this.db.indexes[
									treatedLine.data.fieldName
								] = new Index(treatedLine.data.data);
							}
							if (treatedLine.status === "remove") {
								delete this.db.indexes[
									treatedLine.data.fieldName
								];
							}
							...
							if (treatedLine.status === "add") {
								try {
									...
									if (
										e
											.toString()
											.indexOf(treatedLine.data._id) !==
										-1
									) {
										this.db.removeFromIndexes(
											treatedLine.data
										);
										this.db.addToIndexes(treatedLine.data);
									} else {
				...
				if (
					processed > 0 &&
					corrupt / processed > this.corruptAlertThreshold
				) {
					throw new Error(
		...
		readIndexes(event) {
			return __awaiter(this, void 0, void 0, function* () {
				for (
					let index = 0;
					index < this._memoryIndexes.length;
					index++
				) {
	...
	var priorityQueue = createCommonjsModule(function (module, exports) {
		...
		class PriorityQueue {
			...
			enqueue(run, options) {
				...
				const index = lowerBound_1.default(
					this._queue,
					element,
    ...
    */
		class PQueue extends eventemitter3 {
			constructor(options) {
				...
				if (
					!(
						typeof options.intervalCap === "number" &&
        ...
        */
			...
			set concurrency(newConcurrency) {
				if (
					!(typeof newConcurrency === "number" && newConcurrency >= 1)
				) {
					throw new TypeError(
	...
	class Datastore {
		constructor(options) {
			...
			const PersistenceAdapter =
				options.persistence_adapter || Persistence;
		...
		ensureIndex(options) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				if (options.expireAfterSeconds !== undefined) {
					this.ttlIndexes[options.fieldName] =
		...
		_insert(newDoc) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				yield this.persistence.persistByAppendNewData(
		...
		_update(query, updateQuery, options) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				let upsert =
					options.upsert !== undefined ? options.upsert : false;
				const cursor = new Cursor(this, query);
				cursor.limit(1);
	...
	class Database {
		...
		update({ filter, update, multi }) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				if (!this._datastore) {
					return this._externalCall("update", {
						filter,
						update,
						multi,
					});
		...
		upsert({ filter, update, multi }) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				if (!this._datastore) {
					return this._externalCall("upsert", {
						filter,
						update,
						multi,
					});
		...
		resetAutoCompaction(interval) {
			return __awaiter(this, void 0, void 0, function* () {
				if (!this._datastore) {
					return this._externalCall("resetAutoCompaction", {
						interval,
					});
				}
	...
	class IDB_Persistence_Adapter extends Persistence {
		init() {
			return __awaiter(this, void 0, void 0, function* () {
				databases["data"] = new Store(this.ref + "data", "data");
				databases["indexes"] = new Store(
					this.ref + "indexes",
					"indexes"
				);
			});
