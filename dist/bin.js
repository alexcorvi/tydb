...
var cors = _interopDefault(require('fastify-cors'));
var fs = require('fs');
...
}
{
    ...
    if (!configs) {
        ...
        if (!configs.databases || typeof configs.databases !== "object") {
            console.error(`Error: "databases" in configuration file is required, and should be a string`);
            process.exit(1);
        }
        if (Object.keys(configs.databases).length === 0) {
            console.error(`Error: "databases" in configuration file must have key value property (or properties) that refer to databases`);
            process.exit(1);
        }
        if (!configs.fastify) {
        ...
        if (!configs.fastify.listen) {
            console.error(`Error: "fastify.listen" in configuration file is required`);
            process.exit(1);
        }
        if (!configs.fastify.server) {
            console.error(`Error: "fastify.server" in configuration file is required`);
            process.exit(1);
        }
    }
}
...
Object.keys(configs.databases).forEach((namespace) => {
    if (!configs.databases[namespace]) {
        console.error(`Error: namespace ${namespace} in configuration file does not have a "ref" property, "ref" property is required for all namespaces`);
        process.exit(1);
    }
...
server.listen(configs.fastify.listen, (err, address) => {
    ...
    server.log.info(`Databases are: ${Object.keys(databases)
        .map((x) => address + "/" + x)
        .join(" ")}`);
});
