...
class Datastore<G extends Partial<BaseSchema> & { [key: string]: any }> {
	filename: string = "";
	 ...
	 */
	async loadDatabase() {
		await this.persistence.loadDatabase();
	}
	...
	async getCandidates(query: any, dontExpireStaleDocs?: boolean) {
		const candidates = this.leastCandidates(query);
	 ...
	 */
	private async _insert(newDoc: G) {
	 ...
	 */
	private prepareDocumentForInsertion(newDoc: G) {
		let preparedDoc: G[] | G = [];
	...
	async insert(newDoc: G) {
		await this.q.add(() => this._insert(newDoc));
	}
	...
	private async _update(
		...
		if (res.length > 0) {
			...
			return {
				numberOfUpdated: updatedDocs.length,
				affectedDocuments: updatedDocs.map((x) => model.deepCopy(x)),
				upsert: false,
			};
	 ...
	 */
	private async _remove(query: any, options: { multi: boolean }) {
		let numRemoved = 0;
		...
		this.persistence.persistNewState(removedDocs);
	}

	async remove(query: any, options: { multi: boolean }) {
		return this.q.add(() => this._remove(query, options));
		// TODO: make surethat all this.q.add are returhing a promise
	}
}
