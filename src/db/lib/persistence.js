...
Persistence.getNWAppFilename = function (appName, relativeFilename) {
    ...
    switch (process.platform) {
        ...
        case "linux":
            ...
            break;
        default:
            throw new Error("Can't use the Node Webkit relative path for platform " +
                process.platform);
            break;
    }
...
Persistence.prototype.persistCachedDatabase = function (cb) {
    ...
    Object.keys(this.db.indexes).forEach(function (fieldName) {
        if (fieldName != "_id") {
            ...
            toPersist +=
                self.afterSerialization(model.serialize({
                    $$indexCreated: {
                        fieldName: fieldName,
    ...
    async.waterfall([
        function (cb) {
            Persistence.ensureDirectoryExists(path.dirname(self.filename), function (err) {
                storage_1.default
                    .ensureDataFileIntegrity(self.filename)
                    .then(function () {
                    storage_1.default
                        .readFile(self.filename, "utf8")
