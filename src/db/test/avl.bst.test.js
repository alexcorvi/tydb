var should = require("chai").should(),
	...
	BinarySearchTree = require("../lib/avl/bst").default,
	_ = require("underscore"),
	customUtils = require("../lib/avl//utils");
describe("Binary search tree", function () {
	...
	describe("Deletion", function () {
		...
		it("Can delete a non-root node that has two children", function () {
			...
			[10, 3, 1, 4, 8, 6, 9, 15, 12, 11, 13, 20, 19, 42].forEach(
				function (k) {
					_.isEqual(bst.search(k), ["some " + k]).should.equal(true);
				}
			);
			...
			bst.checkIsBST();
			bst.getNumberOfKeys().should.equal(14);
			[10, 5, 3, 1, 4, 8, 6, 9, 12, 11, 13, 20, 19, 42].forEach(function (
				k
			) {
				_.isEqual(bst.search(k), ["some " + k]).should.equal(true);
			});
