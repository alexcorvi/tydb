...
describe("Database", () => {
	...
	describe("Insert", () => {
		...
		it("If timestampData option not set, don't create a createdAt and a updatedAt field", (done) => {
			d.insert({ hello: "world" }).then((insertedDoc) => {
		...
		it("Array fields match if any element matches", (done) => {
			d.insert({
				...
				d.insert({
					...
					d.insert({ fruits: ["banana"] }).then(function (doc3) {
						d.find({ fruits: "pear" })
							.then(function (docs) {
								docs.length.should.equal(2);
	...
	describe("Update", () => {
		it("If the query doesn't match anything, database is not modified", async () => {
			...
			n.affectedDocuments.length.should.equal(0);
		...
		describe("Upserts", () => {
			it("Can perform upserts if needed", async () => {
				...
				}
				{
					...
					u.numberOfUpdated.should.equal(0);
					assert.isEmpty(u.affectedDocuments);
				}
				{
					...
					update.numberOfUpdated.should.equal(1);
					assert.isNotEmpty(update.affectedDocuments);
		...
		it("Can update documents using multiple modifiers", async () => {
			...
			u.numberOfUpdated.should.equal(1);
		...
		it("If an error is thrown by a modifier, the database state is not changed", async () => {
			...
			const doc = await db.insert({ hello: "world" });
		...
		it("Cant change the _id of a document", async () => {
			const _id = (await d.insert({ a: 2 }))._id;
			await expect(
				d.update(
					{ a: 2 },
					{
		...
		it("return all updated docs", async () => {
			...
			b1.affectedDocuments.length.should.equal(0);
			...
			b2.affectedDocuments[0].a.should.equal(5);
			...
			b3.numberOfUpdated.should.equal(3);
			b3.affectedDocuments[0].b.should.equal(1);
		});
	...
	describe("Remove", () => {
		it("Can remove multiple documents", async () => {
			...
			).numberOfUpdated.should.equal(5);
					...
					d.insert({
						...
						d.update(
							...
							.then(function (nr) {
								...
								nr.numberOfUpdated.should.equal(1);
								data.length.should.equal(2);
								assert.deepEqual(doc1, {
									a: 456,
									b: "no",
							...
							.then(function (nr) {
								...
								nr.numberOfUpdated.should.equal(2);
								data.length.should.equal(2);
								assert.deepEqual(doc1, {
									a: 466,
									b: "same",
		...
		describe("Persisting indexes", function () {
			it("Indexes are persisted to a separate file and recreated upon reload", function (done) {
				...
				db.loadDatabase().then(() => {
					...
					db.insert({ planet: "Earth" }).then(function (err) {
						db.insert({ planet: "Mars" }).then(function (err) {
